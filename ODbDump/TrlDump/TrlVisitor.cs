...
public class TrlVisitor : ITrlVisitor
{
    ...
    public void ModifyRelationInKeyValue(ulong relationIdx, int keyLength, int prefixLength)
    {
        OperationDetail($"Relation id {relationIdx} keyLength: {keyLength} prefixLength: {prefixLength}");
        AddSize(_relationSizes, relationIdx, keyLength);
    }
