...
namespace BTDBTest
{
    public abstract class KeyValueDBTestBase
    {
        ...
        protected abstract IKeyValueDB NewKeyValueDB(IFileCollection fileCollection, ICompressionStrategy compression, uint fileSplitSize = int.MaxValue);

        protected abstract IKeyValueDB NewKeyValueDB(IFileCollection fileCollection, ICompressionStrategy compression, uint fileSplitSize,
        ...
        public void BiggerKey(int keyLength)
        {
            ...
            using (var tr2 = db.StartTransaction())
            {
                ...
                Assert.Equal(key.ToArray(), tr2.GetKeyAsReadOnlySpan().ToArray());
            }
        }
        ...
        public void SimpleFindNextKeyWorks()
        {
            ...
            using (var tr2 = db.StartTransaction())
            {
                ...
                Assert.Equal(Key2, tr2.GetKeyAsReadOnlySpan().ToArray());
        ...
        public void CreateOrUpdateKeyValueWorks(int length)
        {
            ...
            {
                Assert.True(tr2.FindExactKey(_key1));
                var valueBuf2 = tr2.GetValueAsReadOnlySpan().ToArray();
                ...
                Assert.True(tr2.FindExactKey(Key2));
                valueBuf2 = tr2.GetValueAsReadOnlySpan().ToArray();
                for (var i = 0; i < length; i++)
                {
                    if (valueBuf[i] != valueBuf2[i])
                        Assert.Equal(valueBuf[i], valueBuf2[i]);
        ...
        public void FindLastKeyWorks()
        {
            ...
            Assert.Equal(Key2, tr.GetKeyAsReadOnlySpan().ToArray());
            tr.Commit();
        }
        ...
        public void SimplePrefixWorks()
        {
            ...
            Assert.Equal(_key1.AsSpan(0, 3).ToArray(), tr.GetKeyAsReadOnlySpan().ToArray());
        ...
        public void MoreComplexReopen()
        {
            ...
            using (var db = NewKeyValueDB(fileCollection))
            {
                using (var tr = db.StartTransaction())
                {
                    ...
                    Assert.Equal(5, tr.GetKeyAsReadOnlySpan()[1]);
