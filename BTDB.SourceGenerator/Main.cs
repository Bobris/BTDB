...
public class SourceGenerator : IIncrementalGenerator
{
            ...
            (syntaxContext, _) =>
            {
                try
                {
                    ...
                    if (syntaxContext.Node is AttributeSyntax attributeSyntax)
                    {
                        ...
                        if (symb.AllInterfaces.FirstOrDefault(IsICovariantRelation) is { } relation)
                        {
                            return GenerationInfoForRelationInterface(symb, relation.TypeArguments[0], semanticModel);
                        }
                    ...
                    if (syntaxContext.Node is InterfaceDeclarationSyntax)
                    {
                        ...
                        {
                            return GenerationInfoForRelationInterface(symbol, relation.TypeArguments[0], semanticModel);
                        }
                        else
                        {
    ...
    GenerationInfo? GenerationInfoForRelationInterface(INamedTypeSymbol symbol, ITypeSymbol relationType,
        SemanticModel semanticModel)
    {
        if (relationType is not INamedTypeSymbol namedRelationType)
        {
            return null;
        }
        ...
        var generationInfo = GenerationInfoForClass(namedRelationType, null, false, null, semanticModel,
            [], [], true);
        if (generationInfo is not { GenType: GenerationType.Class }) return null;
        var detectedError = DetectErrors(generationInfo.Fields.AsSpan(), symbol.Locations[0]);
    ...
    static GenerationInfo? GenerationInfoForClass(INamedTypeSymbol symbol,
        ...
        HashSet<CollectionInfo> collections, HashSet<GenerationInfo> nested, bool isRelationItem,
                ...
                else
                {
                             ...
                             (ITypeSymbol)namedTypeSymbol is INamedTypeSymbol namedTypeSymbol1)
                    {
                        var gi = GenerationInfoForClass(namedTypeSymbol1, null, false, null, model,
                            collections, nested, false, processed);
                        if (gi != null)
                            nested.Add(gi);
                    }
                }
    ...
    static void GenerateStructStackAllocations(SourceProductionContext context, HashSet<GenerationInfo> structs)
    {
        ...
        factoryCode.Append($$"""
            // <auto-generated/>
            #nullable enable
            #pragma warning disable 612, 618, CS0649, CS8500, SYSLIB0057
            using System;
    ...
    static void GenerateClassFactory(SourceProductionContext context, GenerationInfo generationInfo)
    {
        ...
        var reuseConstructorAccessorForMetadata =
            generationInfo.PrivateConstructor &&
            generationInfo.HasDefaultConstructor &&
        ...
        var nameWithGeneric = "";
        if (!IsReflectionMetadataExcluded(generationInfo.FullName))
        {
            if (!generationInfo.GenericParameters.IsEmpty)
            {
                ...
                {
                    if (!reuseConstructorAccessorForMetadata)
                    {
                        // language=c#
                        declarations.Append($"""
                                [UnsafeAccessor(UnsafeAccessorKind.Constructor)]
            ...
            metadataCode.Append($$"""
                        ...
                        metadata.Creator = &{{(reuseConstructorAccessorForMetadata ? "" : ActivatorName(generationInfo))}}{{(reuseConstructorAccessorForMetadata ? "Constr" : "Creator")}};
    ...
    static string GenericConstrains(IEnumerable<GenericParameter> genericParametersSource)
    {
        ...
        return
            $" where {string.Join(" where ", genericParameters.Where(SomeConstraint).Select(p => $"{p.Name}: " + string.Join(", ", EnumerateConstraints(p))))}";
    }
