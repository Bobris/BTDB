    ...
    internal sealed class LowLevelDBTransaction : ILowLevelDBTransaction
    {
        ...
        public FindKeyResult FindKey(byte[] keyBuf, int keyOfs, int keyLen, FindKeyStrategy strategy)
        {
            ...
            try
            {
                ...
                if (iter.Count >= 126 || _owner.Tweaks.ShouldSplitBTreeChild(iter.TotalLength, additionalLengthNeeded, iter.Count))
                {
        ...
        void AddToBTreeParent(Sector leftSector, Sector rightSector, byte[] middleKeyData, int middleKeyLen, int middleKeyOfs, int middleKeyLenInSector)
        {
            ...
            byte[] mergedData;
            if (iter.Count < 125 && !_owner.Tweaks.ShouldSplitBTreeParent(iter.TotalLength, additionalLengthNeeded, iter.Count + 1))
            {
        ...
        public void SetValueSize(long newSize)
        {
            ...
            if (oldDeepSize != newDeepSize)
            {
                ...
                {
                    SectorPtr.Pack(_currentKeySector.Data, iter.ValueOffset + newInlineSize,
                                   ResizeContentSector(oldValueSectorPtr, oldDeepSize, _currentKeySector, newDeepSize));
                }
            }
        ...
        SectorPtr ResizeContentSector(SectorPtr oldSectorPtr, long oldSize, Sector parentSector, long newSize)
        {
            ...
            try
            {
                ...
                {
                    return ResizeContentSectorChild(ref sector, oldSectorPtr, oldDownPtrCount, newDownPtrCount, parentSector);
                }
                sector = _owner.TryGetSector(oldSectorPtr.Ptr);
                if (sector == null)
                {
        ...
        SectorPtr ResizeContentSectorChild(ref Sector sector, SectorPtr oldSectorPtr, int oldDownPtrCount, int newDownPtrCount, Sector parentSector)
        {
            sector = _owner.TryGetSector(oldSectorPtr.Ptr);
            if (sector == null)
            {
        ...
        void SimplifyBTree(ref Sector sector, int mergeAroundIndex)
        {
            ...
            try
            {
                ...
                if (leftSector.Type == SectorType.BTreeChild)
                {
                    ...
                    if (leftIter.Count + rightIter.Count >= 126) return;
                ...
                {
                    var keyStorageLen = iter.ChildSectorPtrOffset - entryOffset;
                ...
                PackUnpack.PackInt64(sector.Data, entryOffset - 8, mergedPairs);
