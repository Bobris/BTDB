...
public class AppendingEventStore : ReadOnlyEventStore, IWriteEventStore
{
    ...
    public void Store(object metadata, IReadOnlyList<object> events)
    {
        ...
        SerializeIntoBuffer(metadata, events, out var startOffset, out var serializerContext, out var blockType,
            out var lenWithoutEndPadding, out var block);
        if (SpaceNeeded(startOffset, lenWithoutEndPadding) + EndBufferPosition + EndBufferLen > File.MaxFileSize)
        {
            ...
            Mapping.Reset();
            SerializeIntoBuffer(metadata, events, out startOffset, out serializerContext, out blockType,
                out lenWithoutEndPadding, out block);
    ...
    public ulong KnownAppendablePosition()
    {
        if (!IsKnownAsAppendable())
            throw new InvalidOperationException(
                "IsKnownAsAppendable needs to return true before calling this method. Use ReadToEnd(new SkippingEventObserver()).Wait() to initialize.");
    ...
    void WriteOneBlock(ByteBuffer block, BlockType blockType)
    {
        ...
        block = ByteBuffer.NewAsync(block.Buffer, o,
            (int)((uint)(lenWithoutEndPadding + SectorSize - 1) & SectorMaskUInt));
