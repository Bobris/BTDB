...
namespace BTDB.KVDBLayer
{
    public class ArtKeyValueDB : IHaveSubDB, IKeyValueDBInternal
    {
        ...
        IRootNode _nextRoot;
        ArtKeyValueDBTransaction _writingTransaction;
        ...
        public ArtKeyValueDB(IFileCollection fileCollection, ICompressionStrategy compression,
            uint fileSplitSize = int.MaxValue)
            : this(fileCollection, compression, fileSplitSize, CompactorScheduler.Instance)
        {
        }

        public ArtKeyValueDB(IFileCollection fileCollection, ICompressionStrategy compression, uint fileSplitSize,
        ...
        public ArtKeyValueDB(KeyValueDBOptions options)
        {
        ...
        void LoadInfoAboutFiles(ulong? openUpToCommitUlong)
        {
            ...
            try
            {
                ...
                if (openUpToCommitUlong.HasValue || latestTrLogFileId != firstTrLogId && firstTrLogId != 0 || !hasKeyIndex && _fileCollection.FileInfos.Any(p => p.Value.SubDBId == 0))
                {
        ...
        bool LoadTransactionLog(uint fileId, uint logOffset, ulong? openUpToCommitUlong)
        {
            ...
            try
            {
                ...
                var cursor = _lastCommited.CreateCursor();
                var cursor2 = _lastCommited.CreateCursor();
                if (_nextRoot != null)
                {
        ...
        void NewWritingTransactionUnsafe(TaskCompletionSource<IKeyValueDBTransaction> tcs)
        {
            ...
            try
            {
                _writingTransaction = new ArtKeyValueDBTransaction(this, newTransactionRoot, true, false);
            }
            catch
            {
        ...
        internal long ReplaceBTreeValues(CancellationToken cancellation, Dictionary<ulong, ulong> newPositionMap)
        {
            ...
            while (true)
            {
                ...
                {
                    var newRoot = (tr as ArtKeyValueDBTransaction).ArtRoot;
                    var cursor = newRoot.CreateCursor();
                    if (restartKey != null)
                    {
                        cursor.Find(new ReadOnlySpan<byte>(), restartKey);
                        cursor.MovePrevious();
                    }
