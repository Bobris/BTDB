...
class Cursor12 : ICursor
{
    ...
    public unsafe ReadOnlySpan<byte> GetKeyParts(out ReadOnlySpan<byte> keySuffix)
    {
        if (_stack.Count == 0)
        {
            keySuffix = ReadOnlySpan<byte>.Empty;
        ...
        if (header.HasLongKeys)
        {
            ...
            keySuffix = new Span<byte>((keyPtr + 4).ToPointer(), lenSuffix);
        }
        else
        {
            ...
            keySuffix = keySuffixes.Slice(ofs, lenSuffix);
        }

        return NodeUtils12.GetPrefixSpan(stackItem._node);
    }
    ...
    public unsafe ReadOnlyMemory<byte> GetKeyMemory(ref Memory<byte> buffer, bool copy)
    {
        ...
        if (header.HasLongKeys)
        {
            ...
            var res = buffer[..len];
            NodeUtils12.GetPrefixSpan(stackItem._node).CopyTo(res.Span);
            new Span<byte>((keyPtr + 4).ToPointer(), lenSuffix).CopyTo(res.Span[header._keyPrefixLength..]);
            return res;
        }
        else
        {
            ...
            var res = buffer[..len];
            NodeUtils12.GetPrefixSpan(stackItem._node).CopyTo(res.Span);
            keySuffixes.Slice(ofs, lenSuffix).CopyTo(res.Span[header._keyPrefixLength..]);
            return res;
        }
    }

    public ReadOnlySpan<byte> GetKeySpan(ref Memory<byte> buffer, bool copy)
    {
        return GetKeyMemory(ref buffer, copy).Span;
    }
