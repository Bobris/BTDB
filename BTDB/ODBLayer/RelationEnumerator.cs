...
namespace BTDB.ODBLayer
{
    ...
    public class RelationAdvancedEnumerator<T> : IEnumerator<T>
    {
        ...
        public static uint CalcCount(RelationDBManipulator<T> manipulator,
            ...
            KeyProposition endKeyProposition, ByteBuffer endKeyBytes)
        {
            ...
            var keyValueTr = tr.KeyValueDBTransaction;
            var keyValueTrProtector = tr.TransactionProtector;

            if (endKeyProposition == KeyProposition.Included)
                endKeyBytes = FindLastKeyWithPrefix(prefixBytes, endKeyBytes, keyValueTr, keyValueTrProtector);

            keyValueTrProtector.Start();
            keyValueTr.SetKeyPrefix(prefixBytes);

            long startIndex;
            long endIndex;
            if (endKeyProposition == KeyProposition.Ignored)
            {
                endIndex = keyValueTr.GetKeyValueCount() - 1;
            }
            else
            {
                switch (keyValueTr.Find(endKeyBytes))
                {
                    case FindResult.Exact:
                        endIndex = keyValueTr.GetKeyIndex();
                    ...
                    case FindResult.Next:
                        endIndex = keyValueTr.GetKeyIndex() - 1;
                        break;
            ...
            {
                switch (keyValueTr.Find(startKeyBytes))
                {
                    case FindResult.Exact:
                        startIndex = keyValueTr.GetKeyIndex();
                        ...
                        break;
                    case FindResult.Previous:
                        startIndex = keyValueTr.GetKeyIndex() + 1;
                        break;
                    case FindResult.Next:
                        startIndex = keyValueTr.GetKeyIndex();
                        break;
            ...
            return (uint)Math.Max(0, endIndex - startIndex + 1);
        }
